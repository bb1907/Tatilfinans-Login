...
export default function BudgetDetail() {
  ...
  return (
    <div className="space-y-6 px-4 pb-10">
      {/* Hero Header */}
      <section className="relative h-48 rounded-[2.5rem] overflow-hidden bg-[#002b36]">
        ...
        <div className="absolute bottom-6 left-8">
          <p className="text-[10px] font-bold text-white/60 uppercase tracking-widest mb-1">Seyahatlerim › Avrupa Turu</p>
          <h1 className="text-3xl font-black text-white font-headline tracking-tight">Paris Bütçe Planı</h1>
        </div>
      </section>
      ...
      <section className="bg-red-50 border border-red-100 rounded-3xl p-5 flex items-start gap-4">
        ...
        <button className="text-red-900 text-[10px] font-black underline uppercase tracking-widest mt-1">Detay</button>
      </section>
        ...
        <div className="w-full space-y-4">
          {categories.map((cat) => (
            <div key={cat.name} className="flex items-center justify-between">
              ...
              <span className="text-sm font-black text-slate-800">%{cat.percent}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Balance Card */}
      <section className="bg-[#4f5e7f] rounded-[2.5rem] p-8 text-white shadow-xl shadow-slate-200">
        <p className="text-[10px] font-bold text-white/60 uppercase tracking-widest mb-1">Kalan Borç/Alacak</p>
        <h2 className="text-4xl font-black mb-6">€420.00</h2>
        ...
        <button className="w-full bg-[#b7eaff] text-[#00647c] h-14 rounded-2xl font-black flex items-center justify-center gap-3 active:scale-95 transition-all">
          Ödemeleri Dengele
          <ArrowRight size={20} />
        </button>
      </section>
